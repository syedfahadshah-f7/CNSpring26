"""
DNS Implementation Assignment - CS3001 Computer Networks
FAST-NUCS, Spring 2026

This module implements a DNS simulation system with Root, TLD, and Authoritative DNS servers.
Demonstrates both iterative and recursive DNS resolution with local caching.

Author: [Student Name]
Student ID: [Student ID]
Section: [Section]
Date: March 27, 2026
"""

from typing import Dict, List, Tuple, Optional
from dataclasses import dataclass, field
from enum import Enum
from datetime import datetime
import random


# ==================== DNS MESSAGE FORMAT ====================
class RecordType(Enum):
    """DNS Record Types"""
    A = "A"          # Address record
    NS = "NS"        # Nameserver record
    MX = "MX"        # Mail exchange record
    CNAME = "CNAME"  # Canonical name


@dataclass
class DNSQuestion:
    """Represents a DNS question in query"""
    domain_name: str
    record_type: RecordType


@dataclass
class DNSRecord:
    """Represents a DNS Resource Record (RR)"""
    domain_name: str
    record_type: RecordType
    ttl: int
    data: str
    priority: Optional[int] = None  # For MX records


@dataclass
class DNSMessage:
    """DNS Protocol Message Format with 16-bit ID and flags"""
    query_id: int                          # 16-bit identification
    is_query: bool                         # Query (True) or Reply (False)
    is_recursive: bool                     # Recursion desired
    is_authoritative: bool = False         # Is authoritative answer
    is_truncated: bool = False             # Message truncated

    questions: List[DNSQuestion] = field(default_factory=list)
    answers: List[DNSRecord] = field(default_factory=list)
    authority: List[DNSRecord] = field(default_factory=list)
    additional: List[DNSRecord] = field(default_factory=list)

    def __str__(self) -> str:
        direction = "QUERY" if self.is_query else "REPLY"
        flags = f"[Recursive: {self.is_recursive}, Authoritative: {self.is_authoritative}]"
        msg = f"\n--- DNS MESSAGE (ID: {self.query_id:05d}) [{direction}] {flags} ---\n"

        if self.questions:
            msg += f"QUESTIONS ({len(self.questions)}):\n"
            for q in self.questions:
                msg += f"   {q.domain_name} ({q.record_type.value})\n"

        if self.answers:
            msg += f"ANSWERS ({len(self.answers)}):\n"
            for ans in self.answers:
                if ans.priority:
                    msg += f"   {ans.domain_name} {ans.record_type.value} {ans.priority} {ans.data}\n"
                else:
                    msg += f"   {ans.domain_name} {ans.record_type.value} {ans.data}\n"

        if self.authority:
            msg += f"AUTHORITY ({len(self.authority)}):\n"
            for auth in self.authority:
                msg += f"   {auth.domain_name} {auth.record_type.value} {auth.data}\n"

        if self.additional:
            msg += f"ADDITIONAL ({len(self.additional)}):\n"
            for add in self.additional:
                msg += f"   {add.domain_name} {add.record_type.value} {add.data}\n"

        return msg


# ==================== DNS CACHE ====================
class DNSCache:
    """Local DNS Cache with LRU eviction policy"""

    def __init__(self, max_size: int = 5):
        self.max_size = max_size
        self.cache: Dict[Tuple[str, RecordType], List[DNSRecord]] = {}
        self.access_time: Dict[Tuple[str, RecordType], float] = {}
        self.hits = 0
        self.misses = 0

    def put(self, domain: str, record_type: RecordType, records: List[DNSRecord]):
        """Add records to cache"""
        key = (domain, record_type)

        # Remove oldest entry if cache is full
        if len(self.cache) >= self.max_size and key not in self.cache:
            oldest_key = min(self.access_time, key=self.access_time.get)
            del self.cache[oldest_key]
            del self.access_time[oldest_key]
            print(f"    CACHE FLUSH: Removed LRU entry for {oldest_key[0]}")

        self.cache[key] = records
        self.access_time[key] = datetime.now().timestamp()

    def get(self, domain: str, record_type: RecordType) -> Optional[List[DNSRecord]]:
        """Retrieve records from cache"""
        key = (domain, record_type)
        if key in self.cache:
            self.access_time[key] = datetime.now().timestamp()
            self.hits += 1
            return self.cache[key]
        self.misses += 1
        return None

    def is_cached(self, domain: str, record_type: RecordType) -> bool:
        """Check if domain is cached"""
        return (domain, record_type) in self.cache

    def flush(self):
        """Clear entire cache"""
        self.cache.clear()
        self.access_time.clear()


# ==================== AUTHORITATIVE DNS SERVER ====================
class AuthoritativeDNSServer:
    """Stores actual DNS records for authoritative domains"""

    def __init__(self, domain: str):
        self.domain = domain
        self.records: Dict[RecordType, List[DNSRecord]] = {
            RecordType.A: [],
            RecordType.NS: [],
            RecordType.MX: [],
            RecordType.CNAME: []
        }
        self._load_records()

    def _load_records(self):
        """Load real DNS records - hard-coded database"""
        # This is the authoritative database
        dns_database = {
            "google.com": {
                RecordType.A: [
                    ("64.233.187.99", None),
                    ("72.14.207.99", None),
                    ("64.233.167.99", None),
                ],
                RecordType.NS: [
                    ("ns4.google.com.", None),
                    ("ns1.google.com.", None),
                    ("ns2.google.com.", None),
                    ("ns3.google.com.", None),
                ],
                RecordType.MX: [
                    ("smtp4.google.com.", 10),
                    ("smtp1.google.com.", 10),
                    ("smtp2.google.com.", 10),
                    ("smtp3.google.com.", 10),
                ]
            },
            "facebook.com": {
                RecordType.A: [
                    ("31.13.64.35", None),
                    ("31.13.64.36", None),
                ],
                RecordType.NS: [
                    ("a.ns.facebook.com.", None),
                    ("b.ns.facebook.com.", None),
                ],
                RecordType.MX: [
                    ("aspmx.l.google.com.", 10),
                    ("alt1.aspmx.l.google.com.", 20),
                ]
            },
            "github.com": {
                RecordType.A: [
                    ("140.82.113.4", None),
                    ("140.82.114.4", None),
                ],
                RecordType.NS: [
                    ("ns1.github.com.", None),
                    ("ns2.github.com.", None),
                ],
                RecordType.MX: [
                    ("mail.github.com.", 10),
                ]
            },
            "youtube.com": {
                RecordType.A: [
                    ("142.250.185.46", None),
                    ("142.251.41.14", None),
                ],
                RecordType.NS: [
                    ("ns1.youtube.com.", None),
                    ("ns2.youtube.com.", None),
                ],
                RecordType.MX: [
                    ("mail.youtube.com.", 10),
                ]
            },
            "stackoverflow.com": {
                RecordType.A: [
                    ("151.101.1.140", None),
                    ("151.101.65.140", None),
                ],
                RecordType.NS: [
                    ("ns1.stack.com.", None),
                    ("ns2.stack.com.", None),
                ],
                RecordType.MX: [
                    ("mail.stackoverflow.com.", 10),
                ]
            }
        }

        if self.domain in dns_database:
            db = dns_database[self.domain]
            for record_type, records in db.items():
                for data, priority in records:
                    record = DNSRecord(
                        domain_name=self.domain,
                        record_type=record_type,
                        ttl=300,
                        data=data,
                        priority=priority
                    )
                    self.records[record_type].append(record)

    def query(self, domain: str, record_type: RecordType) -> Tuple[List[DNSRecord], bool]:
        """Query this authoritative server"""
        is_authoritative = (domain == self.domain)

        if is_authoritative:
            return self.records.get(record_type, []), True
        return [], False

    def has_domain(self, domain: str) -> bool:
        """Check if this server is authoritative for domain"""
        return domain == self.domain


# ==================== TLD DNS SERVER ====================
class TLDServer:
    """Top-Level Domain (TLD) Server - points to authoritative servers"""

    def __init__(self, tld: str):
        self.tld = tld  # e.g., ".com"
        self.authoritative_servers: Dict[str, AuthoritativeDNSServer] = {}

    def register_domain(self, domain: str, auth_server: AuthoritativeDNSServer):
        """Register a domain with its authoritative server"""
        self.authoritative_servers[domain] = auth_server

    def query(self, domain: str, record_type: RecordType) -> Tuple[Optional[AuthoritativeDNSServer], List[DNSRecord]]:
        """Query TLD server - returns referral to authoritative server"""
        if domain in self.authoritative_servers:
            auth_server = self.authoritative_servers[domain]
            # Return NS records pointing to authoritative server
            ns_records = auth_server.records.get(RecordType.NS, [])
            return auth_server, ns_records
        return None, []


# ==================== ROOT DNS SERVER ====================
class RootDNSServer:
    """Root DNS Server - points to TLD servers"""

    def __init__(self):
        self.tld_servers: Dict[str, TLDServer] = {}

    def register_tld(self, tld: str, tld_server: TLDServer):
        """Register a TLD server"""
        self.tld_servers[tld] = tld_server

    def query(self, domain: str) -> Tuple[Optional[TLDServer], List[DNSRecord]]:
        """Query root server - returns referral to TLD server"""
        # Extract TLD from domain (e.g., "google.com" -> ".com")
        tld = "." + domain.split(".")[-1]

        if tld in self.tld_servers:
            tld_server = self.tld_servers[tld]
            return tld_server, []
        return None, []


# ==================== LOCAL DNS SERVER (RESOLVER) ====================
class LocalDNSServer:
    """Local DNS Server with caching capability"""

    def __init__(self, name: str = "Local Resolver", cache_size: int = 5):
        self.name = name
        self.cache = DNSCache(max_size=cache_size)
        self.root_server: Optional[RootDNSServer] = None
        self.query_counter = 0

    def set_root_server(self, root_server: RootDNSServer):
        """Set reference to root DNS server"""
        self.root_server = root_server

    def resolve_iterative(self, domain: str, record_type: RecordType = RecordType.A) -> Tuple[List[DNSRecord], List[str]]:
        """
        Iterative DNS Resolution
        Client queries each server step-by-step
        """
        self.query_counter += 1
        query_id = random.randint(1, 65535)
        step_log = []

        print(f"\n{'='*70}")
        print(f"[QUERY] ITERATIVE RESOLUTION: {domain} ({record_type.value})")
        print(f"{'='*70}")

        # Check cache first
        if self.cache.is_cached(domain, record_type):
            print(f"[OK] CACHE HIT for {domain}")
            return self.cache.get(domain, record_type), step_log

        print(f"[ERROR] CACHE MISS for {domain}")

        # Step 1: Query Root Server
        print(f"\n[STEP 1] Query ROOT DNS Server for {domain}")
        tld_server, _ = self.root_server.query(domain)
        if not tld_server:
            print(f"  [ERROR] Root server has no information for {domain}")
            return [], step_log
        print(f"  [OK] Root Server: 'Query TLD server for {domain}'")
        step_log.append(f"Root -> TLD Server referral")

        # Step 2: Query TLD Server
        print(f"\n[STEP 2] Query TLD DNS Server for {domain}")
        auth_server, ns_records = tld_server.query(domain, record_type)
        if not auth_server:
            print(f"  [ERROR] TLD server has no information for {domain}")
            return [], step_log
        print(f"  [OK] TLD Server: 'Query Authoritative server {auth_server.domain}'")
        if ns_records:
            for ns in ns_records:
                print(f"    NS Record: {ns.data}")
        step_log.append(f"TLD -> Authoritative Server referral")

        # Step 3: Query Authoritative Server
        print(f"\n[STEP 3] Query AUTHORITATIVE DNS Server for {domain}")
        records, is_auth = auth_server.query(domain, record_type)
        if records:
            print(f"  [OK] Authoritative Server found {len(records)} records:")
            for record in records:
                if record.priority:
                    print(f"    {record.record_type.value}: {record.priority} {record.data}")
                else:
                    print(f"    {record.record_type.value}: {record.data}")
        else:
            print(f"  [ERROR] No records found")
            return [], step_log

        # Cache the result
        self.cache.put(domain, record_type, records)
        print(f"\n  [CACHE] Result cached for future queries")

        return records, step_log

    def resolve_recursive(self, domain: str, record_type: RecordType = RecordType.A) -> Tuple[List[DNSRecord], List[str]]:
        """
        Recursive DNS Resolution
        Client sends one request, DNS servers resolve internally
        """
        self.query_counter += 1
        query_id = random.randint(1, 65535)
        step_log = []

        print(f"\n{'='*70}")
        print(f"[RECURSIVE] RECURSIVE RESOLUTION: {domain} ({record_type.value})")
        print(f"{'='*70}")

        # Check cache first
        if self.cache.is_cached(domain, record_type):
            print(f"[OK] CACHE HIT for {domain}")
            return self.cache.get(domain, record_type), step_log

        print(f"[ERROR] CACHE MISS for {domain}")

        # Create initial query
        query = DNSMessage(
            query_id=query_id,
            is_query=True,
            is_recursive=True,
            questions=[DNSQuestion(domain, record_type)]
        )
        print(f"\n[SEND] Client sends recursive query to root server:")
        print(query)

        # Local server queries root recursively
        print("[LOCAL RESOLVER] Processing recursive query...")

        # Step 1: Query Root
        print(f"\n  [-> ROOT SERVER] Querying for {domain}")
        tld_server, _ = self.root_server.query(domain)
        if not tld_server:
            print(f"    [ERROR] Not found at root level")
            return [], step_log
        print(f"    [OK] Root: Referral to TLD server")
        step_log.append("Root Server -> TLD referral")

        # Step 2: Query TLD
        print(f"\n  [-> TLD SERVER] Querying for {domain}")
        auth_server, ns_records = tld_server.query(domain, record_type)
        if not auth_server:
            print(f"    [ERROR] Not found at TLD level")
            return [], step_log
        print(f"    [OK] TLD: Referral to Authoritative server")
        step_log.append("TLD Server -> Authoritative referral")

        # Step 3: Query Authoritative
        print(f"\n  [-> AUTHORITATIVE SERVER] Querying for {domain}")
        records, is_auth = auth_server.query(domain, record_type)
        if records:
            print(f"    [OK] Found {len(records)} records")
            for record in records:
                if record.priority:
                    print(f"      {record.record_type.value}: {record.priority} {record.data}")
                else:
                    print(f"      {record.record_type.value}: {record.data}")
        else:
            print(f"    [ERROR] No records found")
            return [], step_log

        step_log.append("Authoritative Server -> Answer")

        # Create reply message
        reply = DNSMessage(
            query_id=query_id,
            is_query=False,
            is_recursive=True,
            is_authoritative=True,
            answers=records
        )
        print(f"\n[RECEIVE] Server sends recursive reply to client:")
        print(reply)

        # Cache the result
        self.cache.put(domain, record_type, records)
        print(f"\n  [CACHE] Result cached for future queries")

        return records, step_log


# ==================== DNS CLIENT ====================
class DNSClient:
    """DNS Client for making queries"""

    def __init__(self, local_server: LocalDNSServer):
        self.local_server = local_server

    def resolve_recursive(self, domain: str, record_type: RecordType = RecordType.A) -> List[DNSRecord]:
        """Resolve domain using recursive resolution"""
        return self.local_server.resolve_recursive(domain, record_type)[0]

    def resolve_iterative(self, domain: str, record_type: RecordType = RecordType.A) -> List[DNSRecord]:
        """Resolve domain using iterative resolution"""
        return self.local_server.resolve_iterative(domain, record_type)[0]


# ==================== PRINT UTILITIES ====================
def print_header(student_name: str, student_id: str, section: str):
    """Print assignment header"""
    print("\n" + "="*70)
    print("FAST-NUCS | COMPUTER NETWORKS (CS3001)")
    print("Assignment #2: DNS Implementation")
    print("="*70)
    print(f"Student Name:   {student_name}")
    print(f"Student ID:     {student_id}")
    print(f"Section:        {section}")
    print(f"Date:           {datetime.now().strftime('%B %d, %Y')}")
    print("="*70 + "\n")


def print_dns_records(domain: str, records: List[DNSRecord]) -> str:
    """Print DNS records in required format"""
    if not records:
        return f"No records found for {domain}\n"

    # Get first A record for display
    a_records = [r for r in records if r.record_type == RecordType.A]
    first_ip = a_records[0].data if a_records else "Unknown"

    output = f"{domain}/{first_ip}\n"
    output += "-- DNS INFORMATION --\n"

    # A Records
    a_recs = [r for r in records if r.record_type == RecordType.A]
    if a_recs:
        ips = ", ".join([r.data for r in a_recs])
        output += f"A: {ips}\n"

    # NS Records
    ns_recs = [r for r in records if r.record_type == RecordType.NS]
    if ns_recs:
        ns_list = ", ".join([r.data for r in ns_recs])
        output += f"NS: {ns_list}\n"

    # MX Records
    mx_recs = [r for r in records if r.record_type == RecordType.MX]
    if mx_recs:
        mx_list = ", ".join([f"{r.priority} {r.data}" for r in mx_recs])
        output += f"MX: {mx_list}\n"

    return output


# ==================== MAIN DEMO ====================
def main():
    """Main demonstration of DNS simulation"""

    # Print Header
    print_header(
        student_name="[Syed Fahad Faheem Shah]",
        student_id="[23K-0062]",
        section="[BS-AI-6A]"
    )

    # ============== SETUP DNS INFRASTRUCTURE ==============
    print("\n[DNS] INITIALIZING DNS INFRASTRUCTURE...\n")

    # Create authoritative servers
    auth_google = AuthoritativeDNSServer("google.com")
    auth_facebook = AuthoritativeDNSServer("facebook.com")
    auth_github = AuthoritativeDNSServer("github.com")
    auth_youtube = AuthoritativeDNSServer("youtube.com")
    auth_stackoverflow = AuthoritativeDNSServer("stackoverflow.com")

    print("[OK] Authoritative Servers Created:")
    print("  - google.com")
    print("  - facebook.com")
    print("  - github.com")
    print("  - youtube.com")
    print("  - stackoverflow.com")

    # Create TLD servers
    tld_com = TLDServer(".com")
    tld_com.register_domain("google.com", auth_google)
    tld_com.register_domain("facebook.com", auth_facebook)
    tld_com.register_domain("github.com", auth_github)
    tld_com.register_domain("youtube.com", auth_youtube)
    tld_com.register_domain("stackoverflow.com", auth_stackoverflow)

    print("\n[OK] TLD Server Created: .com")
    print("  Registered domains: google.com, facebook.com, github.com, youtube.com, stackoverflow.com")

    # Create root server
    root_server = RootDNSServer()
    root_server.register_tld(".com", tld_com)

    print("\n[OK] Root DNS Server Created")

    # Create local DNS server with cache
    local_server = LocalDNSServer("dns.poly.edu", cache_size=5)
    local_server.set_root_server(root_server)

    print("[OK] Local DNS Server Created (dns.poly.edu)")
    print("  Cache size: 5 entries (LRU Eviction Policy)\n")

    # Create DNS client
    client = DNSClient(local_server)

    # ============== TEST 1: ITERATIVE RESOLUTION ==============
    print("\n" + ">"*35)
    print("TEST 1: ITERATIVE DNS RESOLUTION")
    print(">"*35)

    records = client.resolve_iterative("google.com", RecordType.A)
    if records:
        print("\n" + "-"*70)
        print(" FINAL RESULT:")
        print("-"*70)
        print(print_dns_records("google.com", records +
              auth_google.records.get(RecordType.NS, []) +
              auth_google.records.get(RecordType.MX, [])))

    # Second query (should hit cache)
    print("\n" + ">"*35)
    print("TEST 2: ITERATIVE RESOLUTION (CACHED)")
    print(">"*35)

    records = client.resolve_iterative("google.com", RecordType.A)

    # ============== TEST 3: RECURSIVE RESOLUTION ==============
    print("\n" + ">"*35)
    print("TEST 3: RECURSIVE DNS RESOLUTION")
    print(">"*35)

    records = client.resolve_recursive("facebook.com", RecordType.A)
    if records:
        print("\n" + "-"*70)
        print(" FINAL RESULT:")
        print("-"*70)
        print(print_dns_records("facebook.com", records +
              auth_facebook.records.get(RecordType.NS, []) +
              auth_facebook.records.get(RecordType.MX, [])))

    # ============== TEST 4: CACHE STRESS TEST ==============
    print("\n" + ">"*35)
    print("TEST 4: CACHE AUTO-FLUSHING (LRU)")
    print(">"*35)
    print(f"\nCache max size: {local_server.cache.max_size}")
    print("Resolving 6 different domains to test LRU eviction...\n")

    domains_to_query = [
        "github.com",
        "youtube.com",
        "stackoverflow.com",
        "google.com",  # Will trigger cache flush
        "facebook.com"
    ]

    for domain in domains_to_query:
        print(f"\n{'-'*70}")
        print(f"Querying: {domain}")
        print(f"{'-'*70}")
        records = client.resolve_iterative(domain, RecordType.A)

  
if __name__ == "__main__":
    main()
